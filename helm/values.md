...
| fluentbit.lua.script | string | `""` | Inline Lua script (tpl-rendered, so it can reference `.Release.Name`, chart values, etc.) mounted at /fluent-bit/scripts/custom.lua. One script serves both calls. Only used when enabled. |
