...
| fluentbit.lua.enabled | bool | `false` | Run a custom Lua script over the access-log records selected for forwarding — anything the chart's own values cannot express. Mounted at /fluent-bit/scripts/custom.lua and invoked by a `lua` filter placed after the forwarding grep. Off by default. |
