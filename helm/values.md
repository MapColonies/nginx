...
| fluentbit.debug.logLevel | string | `"debug"` | Fluent Bit's service log level while debug is enabled. `debug` reports plugin-level decisions, `trace` is extremely verbose. When debug is disabled the level is always `info`. |
...
| fluentbit.lua.enabled | bool | `false` | Mount a custom Lua script into the Fluent Bit sidecar for advanced filtering (e.g. latency-threshold forwarding) with no chart change. When enabled, the script is added to the Fluent Bit ConfigMap and mounted at /fluent-bit/scripts/custom.lua; reference it from a `- name: lua` filter entry you supply through fluentbit.accessLog.metrics.config (the chart never inserts one). When disabled (the default), no Lua key or volumeMount is rendered. |
